import getExcelSheets from "./libs/convExcel";
import fs from "fs";
import getDataArr from "./libs/getDataArr"
import num2Alph from "./libs/numberToAph"
const offset = 6;
const modeCode = getExcelSheets.forEach(data => { // 제원표의 modecode. ex) T, TN, TY3N ...
...
getExcelSheets.forEach( (data, index) => { // 제원표의 modecode. ex) T, TN, TY3N ...
  const codeName = /N/g.exec(data.split('_')[0]);
  if(codeName !== null){
    const Ncode = codeName.input;
    if(!(/F/g.test(Ncode))){  // luffing mode
...
// const heightOfHookCrain = {  // Default Hook and Crain height. heightOfHookCrain
//   crainHeight : 2,
//   hookHeight : 6,
// };

// const parseData = () => {
//   getTableData.forEach((sheetName) => {
//     const modeCode = sheetName.split('_')[0];
//     // modeCode를 N과 F로 나눈 코드 작성. F가 없는 N이 luffing, 나머지는 fix.
