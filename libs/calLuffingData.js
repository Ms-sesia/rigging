        ...
        if (workValue.workBuilding.height < workValue.block.height1) {
          ...
          if (params.d2 < B1B2WDistance && B1B2WDistance < spec.distance[i] - craneDistance && blockAngle < spec.mainAngle && minMainAngle < spec.mainAngle) {
            ...
            // 2] h1 + 크레인높이 < 작업높이
              ...
              const blockLuffingAngle = Number(( Math.atan((workValue.workBuilding.height - params.h1 - heightOfHookCrane.craneHeight) / (params.d2 - workValue.workBuilding.vertical)) * (180 / Math.PI)).toFixed(1)); // 픽스(러핑) 시작지점에서 건물까지의 대각선 각도
              if (blockLuffingAngle < params.luffingAngle) {
                const testCode = 5;
                // console.log(testCode);
                return riggingData(spec, i, workValue, heightOfHookCrane, craneDistance, params, testCode);
