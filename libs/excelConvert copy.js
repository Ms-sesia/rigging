...
const getTableInfo = filelist.map((filename) => {
  const workbook = XLSX.readFile(`${cwd}/${filename}`); // filename별 데이터를 불러온다.(시트 전체)
  let excelLength = [];
  workbook.SheetNames.forEach((sheetname, index) => {
    ...
    const tableColumnLength = Object.keys(XLSX.utils.sheet_to_json(workbook.Sheets[sheetname], { header: "A" })[0]).length; //선택한 시트의 열의 개수
    // console.log(tableRowLength, tableColumnLength);
    excelLength[index] = {
      row: tableRowLength,
